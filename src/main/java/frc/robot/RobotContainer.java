...
public class RobotContainer {
  ...
  private void configureBindings() {
    drivetrain.setDefaultCommand( // Drivetrain will execute this command periodically
        drivetrain.applyRequest(() -> drive
            .withVelocityX(-drivercontroller.getLeftY() * Constants.MAX_SPEED) // Drive forward with negative Y
                                                                               // (forward)
    ...
    m_climber.setDefaultCommand(new ClimberCommand(m_climber, () -> operatercontroller.getLeftY(),
        () -> operatercontroller.getRightY(), () -> operatercontroller.leftTrigger(0.5).getAsBoolean()));
    ...
    operatercontroller.x()
        .whileTrue(Commands.startEnd(() -> m_intake.autoIntake(), () -> m_intake.stopIntakeAndFeed()));
    operatercontroller.rightBumper()
        .whileTrue(Commands.startEnd(() -> m_intake.intakeMotorPower(-0.6), () -> m_intake.intakeMotorPower(0.0)));
    drivercontroller.b()
        .whileTrue(Commands.startEnd(() -> m_intake.feedMotorPower(0.6), () -> m_intake.feedMotorPower(0.0)));
    operatercontroller.leftBumper().whileTrue(
        Commands.race(Commands.startEnd(() -> m_shooter.velocityRPM = 1000, () -> m_shooter.velocityRPM = 3000),
  ...
  public RobotContainer() {
    ...
    NamedCommands.registerCommand("FeedOn", Commands.race(new WaitCommand(0.1),
        Commands.startEnd(() -> m_intake.feedMotorPower(0.5), () -> m_intake.feedMotorPower(0))));
